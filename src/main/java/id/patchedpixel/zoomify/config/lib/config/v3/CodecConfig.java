...
import id.patchedpixel.zoomify.config.lib.config.v3.*;
import id.patchedpixel.zoomify.config.lib.config.v3.CodecConfigEntryImpl;
import id.patchedpixel.zoomify.config.lib.config.v3.ConfigEntry;
import id.patchedpixel.zoomify.config.lib.config.v3.EntryAddable;
...
public abstract class CodecConfig<S extends CodecConfig<S>> implements EntryAddable, Codec<S> {
    ...
    public <T extends CodecConfig<T>> ReadonlyConfigEntry<T> register(String fieldName, T configInstance) {
        ReadonlyConfigEntry<T> entry = new id.patchedpixel.zoomify.config.lib.config.v3.ChildConfigEntryImpl<>(fieldName, configInstance);
        entries.add(entry);
        return entry;
    }
